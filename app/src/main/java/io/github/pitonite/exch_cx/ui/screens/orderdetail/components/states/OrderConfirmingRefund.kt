...
@Composable
fun OrderConfirmingRefund(
    order: Order,
) {
  OrderStateCard {
    Text(
        stringResource(R.string.order_state_confirming_refund_title),
    ...
    if (order.toAmount != null) {
      Text(stringResource(R.string.you_will_be_refunded, order.toAmount, order.fromCurrency))
    }
...
@Preview
@Composable
fun OrderConfirmingRefundPreview() {
  ExchTheme {
    Surface {
      OrderConfirmingRefund(
          order = orderConfirmingRefund,
      )
    }
  }
}
